...
def run(event, context) -> []:
  ...
  if context.var.get('out_file') and context.var.get('serializer'):
    print(f'serializing results as {context.var.serializer}:: {context.var.out_file}')
    serialize_func = getattr(context.serialize, context.var.serializer)
    serialize_func(records, context.var.out_file, context.var.newline, context.var.delimiter)
